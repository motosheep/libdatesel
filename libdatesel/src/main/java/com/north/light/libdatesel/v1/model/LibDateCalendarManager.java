...
public class LibDateCalendarManager {
    ...
    private static class SingleHolder {
        static LibDateCalendarManager mInstance = new LibDateCalendarManager();
    }
     ...
     * @return Map
     */
    public Map<String, LibDateMonthInYearDetailInfo> getMonthByYear(String year) throws Exception {
        Date time = new SimpleDateFormat("yyyy").parse(year);
        Calendar calendar = Calendar.getInstance();
        calendar.setTime(time);
        Map<String, LibDateMonthInYearDetailInfo> result = new HashMap<>();
        //计算每个月的天数，星期几，
        //一年一定是从1月开始，到12月结束的
        SimpleDateFormat mDayFormat = new SimpleDateFormat("yyyy-MM-dd");
        Calendar cacheCal = Calendar.getInstance();
        for (int i = 1; i <= 12; i++) {
            LibDateMonthInYearDetailInfo detailInfo = new LibDateMonthInYearDetailInfo();
            ...
            List<LibDateDayInMonthDetailInfo> monthList = getDayByMonth(year, getFixString(i));
     ...
     */
    public List<LibDateDayInMonthDetailInfo> getDayByMonth(String year, String month) throws Exception {
        ...
        List<LibDateDayInMonthDetailInfo> result = new ArrayList<>();
        for (int i = 1; i <= dayOfMonth; i++) {
            ...
            if (startMode == 1) {
                ...
                if (i == 1 && dayOfWeek != 1) {
                    ...
                    for (int sup = 0; sup < supplyCount; sup++) {
                        LibDateDayInMonthDetailInfo supCache = new LibDateDayInMonthDetailInfo();
            ...
            //正常数据---------------
            LibDateDayInMonthDetailInfo normalCache = new LibDateDayInMonthDetailInfo();
