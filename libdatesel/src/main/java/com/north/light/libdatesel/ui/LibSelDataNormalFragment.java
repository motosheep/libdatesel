...
import com.north.light.libdatesel.widget.DateNumberPickerView;
...
public class LibSelDataNormalFragment extends Fragment {
    ...
    private void initEvent() {
        ...
        mConfirm.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                DateMain.getInstance().onSelData(getCurSelTime());
                requireActivity().finish();
            }
        });

        //选择年会影响到日__选择年以后，重置月份，重置日
        mYearPicker.setOnValueChangedListener(new DateNumberPickerView.OnValueChangeListener() {
            @Override
            public void onValueChange(DateNumberPickerView picker, int oldVal, int newVal) {
                mMonthPicker.setValue(0);
                setDayByMonth(mYearNumList.get(newVal), mMonthNumList.get(0));
            }
        });
        //选择月会影响到日
        mMonthPicker.setOnValueChangedListener(new DateNumberPickerView.OnValueChangeListener() {
            @Override
            public void onValueChange(DateNumberPickerView picker, int oldVal, int newVal) {
                //设置日
     ...
     * 获取当前时间
     */
    public DateSelResult getCurSelTime() {
        //数据回调
    ...
    public Long getCurSelTimeStamp() {
        DateSelResult date = getCurSelTime();
        if (date != null) {
