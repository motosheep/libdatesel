...
public class LibDateDateNumberPickerView extends View {
    ...
    public interface OnValueChangeListener {
        void onValueChange(LibDateDateNumberPickerView picker, int oldVal, int newVal);
    }

    public interface OnValueChangeListenerRelativeToRaw {
        void onValueChangeRelativeToRaw(LibDateDateNumberPickerView picker, int oldPickedIndex, int newPickedIndex,
                                        String[] displayedValues);
    }

    public interface OnValueChangeListenerInScrolling {
        void onValueChangeInScrolling(LibDateDateNumberPickerView picker, int oldVal, int newVal);
    }
    ...
    public interface OnScrollListener {
        ...
        void onScrollStateChange(LibDateDateNumberPickerView view, int scrollState);
    }
    ...
    public LibDateDateNumberPickerView(Context context) {
        super(context);
        init(context);
    }

    public LibDateDateNumberPickerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttr(context, attrs);
        init(context);
    }

    public LibDateDateNumberPickerView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.DateNumberPickerView);
        int n = a.getIndexCount();
        for (int i = 0; i < n; i++) {
            int attr = a.getIndex(i);
            if (attr == R.styleable.DateNumberPickerView_npv_ShownCount) {
                mShownCount = a.getInt(attr, DEFAULT_SHOWN_COUNT);
            } else if (attr == R.styleable.DateNumberPickerView_npv_DividerColor) {
                mDividerColor = a.getColor(attr, DEFAULT_DIVIDER_COLOR);
            } else if (attr == R.styleable.DateNumberPickerView_npv_DividerHeight) {
                mDividerHeight = a.getDimensionPixelSize(attr, DEFAULT_DIVIDER_HEIGHT);
            } else if (attr == R.styleable.DateNumberPickerView_npv_DividerMarginLeft) {
                mDividerMarginL = a.getDimensionPixelSize(attr, DEFAULT_DIVIDER_MARGIN_HORIZONTAL);
            } else if (attr == R.styleable.DateNumberPickerView_npv_DividerMarginRight) {
                mDividerMarginR = a.getDimensionPixelSize(attr, DEFAULT_DIVIDER_MARGIN_HORIZONTAL);
            } else if (attr == R.styleable.DateNumberPickerView_npv_TextArray) {
                mDisplayedValues = convertCharSequenceArrayToStringArray(a.getTextArray(attr));
            } else if (attr == R.styleable.DateNumberPickerView_npv_TextColorNormal) {
                mTextColorNormal = a.getColor(attr, DEFAULT_TEXT_COLOR_NORMAL);
            } else if (attr == R.styleable.DateNumberPickerView_npv_TextColorSelected) {
                mTextColorSelected = a.getColor(attr, DEFAULT_TEXT_COLOR_SELECTED);
            } else if (attr == R.styleable.DateNumberPickerView_npv_TextColorHint) {
                mTextColorHint = a.getColor(attr, DEFAULT_TEXT_COLOR_SELECTED);
            } else if (attr == R.styleable.DateNumberPickerView_npv_TextSizeNormal) {
                mTextSizeNormal = a.getDimensionPixelSize(attr, sp2px(context, DEFAULT_TEXT_SIZE_NORMAL_SP));
            } else if (attr == R.styleable.DateNumberPickerView_npv_TextSizeSelected) {
                mTextSizeSelected = a.getDimensionPixelSize(attr, sp2px(context, DEFAULT_TEXT_SIZE_SELECTED_SP));
            } else if (attr == R.styleable.DateNumberPickerView_npv_TextSizeHint) {
                mTextSizeHint = a.getDimensionPixelSize(attr, sp2px(context, DEFAULT_TEXT_SIZE_HINT_SP));
            } else if (attr == R.styleable.DateNumberPickerView_npv_MinValue) {
                mMinShowIndex = a.getInteger(attr, 0);
            } else if (attr == R.styleable.DateNumberPickerView_npv_MaxValue) {
                mMaxShowIndex = a.getInteger(attr, 0);
            } else if (attr == R.styleable.DateNumberPickerView_npv_WrapSelectorWheel) {
                mWrapSelectorWheel = a.getBoolean(attr, DEFAULT_WRAP_SELECTOR_WHEEL);
            } else if (attr == R.styleable.DateNumberPickerView_npv_ShowDivider) {
                mShowDivider = a.getBoolean(attr, DEFAULT_SHOW_DIVIDER);
            } else if (attr == R.styleable.DateNumberPickerView_npv_HintText) {
                mHintText = a.getString(attr);
            } else if (attr == R.styleable.DateNumberPickerView_npv_AlternativeHint) {
                mAlterHint = a.getString(attr);
            } else if (attr == R.styleable.DateNumberPickerView_npv_EmptyItemHint) {
                mEmptyItemHint = a.getString(attr);
            } else if (attr == R.styleable.DateNumberPickerView_npv_MarginStartOfHint) {
                mMarginStartOfHint = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_MARGIN_START_OF_HINT_DP));
            } else if (attr == R.styleable.DateNumberPickerView_npv_MarginEndOfHint) {
                mMarginEndOfHint = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_MARGIN_END_OF_HINT_DP));
            } else if (attr == R.styleable.DateNumberPickerView_npv_ItemPaddingVertical) {
                mItemPaddingVertical = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_ITEM_PADDING_DP_V));
            } else if (attr == R.styleable.DateNumberPickerView_npv_ItemPaddingHorizontal) {
                mItemPaddingHorizontal = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_ITEM_PADDING_DP_H));
            } else if (attr == R.styleable.DateNumberPickerView_npv_AlternativeTextArrayWithMeasureHint) {
                mAlterTextArrayWithMeasureHint = a.getTextArray(attr);
            } else if (attr == R.styleable.DateNumberPickerView_npv_AlternativeTextArrayWithoutMeasureHint) {
                mAlterTextArrayWithoutMeasureHint = a.getTextArray(attr);
            } else if (attr == R.styleable.DateNumberPickerView_npv_RespondChangeOnDetached) {
                mRespondChangeOnDetach = a.getBoolean(attr, DEFAULT_RESPOND_CHANGE_ON_DETACH);
            } else if (attr == R.styleable.DateNumberPickerView_npv_RespondChangeInMainThread) {
                mRespondChangeInMainThread = a.getBoolean(attr, DEFAULT_RESPOND_CHANGE_IN_MAIN_THREAD);
            } else if (attr == R.styleable.DateNumberPickerView_npv_TextEllipsize) {
                mTextEllipsize = a.getString(attr);
            } else if (attr == R.styleable.DateNumberPickerView_npv_SelectedItemBackground) {
        ...
        if (currPickedIndex != mPrevPickedIndex && mRespondChangeOnDetach) {
            try {
                if (mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(LibDateDateNumberPickerView.this, mPrevPickedIndex + mMinValue, currPickedIndex + mMinValue);
                }
                if (mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw.onValueChangeRelativeToRaw(LibDateDateNumberPickerView.this, mPrevPickedIndex, currPickedIndex, mDisplayedValues);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private void respondPickedValueChanged(int oldVal, int newVal, Object respondChange) {
        ...
        if (oldVal != newVal) {
            if (respondChange == null || !(respondChange instanceof Boolean) || (Boolean) respondChange) {
                if (mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(LibDateDateNumberPickerView.this, oldVal + mMinValue, newVal + mMinValue);
                }
                if (mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw.onValueChangeRelativeToRaw(LibDateDateNumberPickerView.this, oldVal, newVal, mDisplayedValues);
                }
            }
        }
